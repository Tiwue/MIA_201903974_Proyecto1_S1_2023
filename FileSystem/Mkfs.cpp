...
void Mkfs::executeMkfs() {
    ...
    if(type != 'C'){
        return coutError("Parametro >type no válido", NULL);
    }
    fs = _2_or_3fs(_fs);
...
void Mkfs::crearSistemaArchivos(ParticionesMontadas _mounted){
    ...
    switch (_mounted.type) {
        case 'P':
            part_size = _mounted.particion.part_s;
            part_start = _mounted.particion.part_start;
            break;
        case 'L':
            part_size = _mounted.logica.part_s;
    ...
    super_bloque.s_inode_s = sizeof(InodosTable);
    super_bloque.s_block_s = 64;
